...
class RegisterForm(FlaskForm):
    def validate_email_address(self, email_address_to_check):
        query = "SELECT Top 1 email FROM [User] where email = '{0}'".format(
            email_address_to_check.data)

        data = executeAndReturnOneRow(query)
...
class AddProduct(FlaskForm):
    ...
    description = TextAreaField(
        label='Description', validators=[DataRequired()])
    barcode = StringField(label='Barcode', validators=[
    ...
    cursor.close()
    conn.close()
    category = SelectField('Category', choices=categoryLst,
